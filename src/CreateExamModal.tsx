...
// Isolated editor — keeps NO state; typing only writes to valueRef via TipTap, so the parent
// modal never re-renders while you type. Parent reads valueRef.current on submit. Remounts
// (via key) when a new question is loaded/reset so the initial content is correct.
const IsolatedCustomEditor = memo(function IsolatedCustomEditor({
  initialValue,
  valueRef,
  placeholder,
  minHeight = '350px',
  ...
  useEffect(() => { valueRef.current = initialValue; }, []);
  return (
    <RichTextEditor
      value={initialValue}
      onChange={(v: string) => { valueRef.current = v; }}
      darkMode={false}
...
// Isolated text input/textarea for array rows — local state while typing (no modal re-render);
// commits to the parent array on blur (blur fires before any Add/Delete/Submit click, so the
// parent array stays current). Call sites key by index+value so delete/reorder stay correct.
const IsolatedInput = memo(function IsolatedInput({ initialValue, onCommit, ...rest }: any) {
  const [v, setV] = useState<string>(initialValue ?? '');
  return <input {...rest} value={v} onChange={(e) => setV(e.target.value)} onBlur={() => onCommit(v)} />;
});

const IsolatedTextarea = memo(function IsolatedTextarea({ initialValue, onCommit, ...rest }: any) {
  const [v, setV] = useState<string>(initialValue ?? '');
  return <textarea {...rest} value={v} onChange={(e) => setV(e.target.value)} onBlur={() => onCommit(v)} />;
});

// Search box with its own state + internal 300ms debounce — typing never re-renders the modal;
// only the debounced value is pushed up (which drives the fetch), once the user pauses.
const IsolatedDebouncedSearch = memo(function IsolatedDebouncedSearch({ initialValue, onDebouncedChange, placeholder, className }: any) {
  const [v, setV] = useState<string>(initialValue ?? '');
  useEffect(() => {
    const timer = setTimeout(() => { onDebouncedChange(v); }, 300);
    return () => clearTimeout(timer);
  }, [v]);
  return <input type="text" placeholder={placeholder} value={v} onChange={(e) => setV(e.target.value)} className={className} />;
});

// Memoized student row — only the toggled row re-renders on selection, not all rows.
// brand values are passed as plain strings so they compare equal by value.
const StudentRow = memo(function StudentRow({ student, isSelected, onToggle, avatarGradient, classColor }: any) {
  const initials = student.fullName?.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase() || '?';
  return (
    <div
    ...
    >
      <input
        type="checkbox"
        checked={isSelected}
        onChange={(e) => { e.stopPropagation(); onToggle(student.userId); }}
...
const QuestionBankCard = memo(function QuestionBankCard({ question, index, selectedQuestionIds, expandedQuestionId, getQuestionTypeDisplay, getComplexityColor, isQuestionAlreadyAdded, currentPage, questionsPerPage, setCarouselImages, setCurrentImageIndex, setImageCarouselOpen, setExpandedQuestionId, selectedStarterLang, setSelectedStarterLang, toggleQuestionSelection, brand, copyToClipboard, copiedCode, safeRender, containsHTML }: any) {
                    const isSelected = selectedQuestionIds.has(question.id);
                    const isExpanded = expandedQuestionId === question.id;
                    const typeDisplay = getQuestionTypeDisplay(question.type);
                    const alreadyAddedStatus = isQuestionAlreadyAdded(question.id, question.questionText);
  ...
  return prev.question === next.question
    ...
    && prev.selectedQuestionIds.has(q.id) === next.selectedQuestionIds.has(q.id)
    && (prev.expandedQuestionId === q.id) === (next.expandedQuestionId === q.id)
    && (prev.selectedStarterLang[q.id] || "") === (next.selectedStarterLang[q.id] || "")
    && prev.copiedCode === next.copiedCode
    && prev.isQuestionAlreadyAdded(q.id, q.questionText).isAdded === next.isQuestionAlreadyAdded(q.id, q.questionText).isAdded;
});
...
export default function CreateExamModal({ isOpen, onClose, onSave, existingExam, currentUser, activeCollegeId, activeCollegeName }: CreateExamModalProps) {
  ...
  // Slide-in on mount (modal is now mounted only while open)
  const [slideVisible, setSlideVisible] = useState(false);
  useEffect(() => { const _raf = requestAnimationFrame(() => setSlideVisible(true)); return () => cancelAnimationFrame(_raf); }, []);
  ...
  const [completionPolicy, setCompletionPolicy] = useState<'strict' | 'flexible' | 'window'>('strict');
  const [attemptWindowDays, setAttemptWindowDays] = useState<number>(4);
  ...
  const [customEditorKey, setCustomEditorKey] = useState(0);
  ...
  const customQuestionChapterRef = useRef('');
  ...
  const customQuestionHintRef = useRef('');
  const customQuestionSolutionRef = useRef('');
 ...
 const [customQuestionTestStub, setCustomQuestionTestStub] = useState('');
  ...
  const isQuestionAlreadyAdded = (questionId: string, questionText?: string): { isAdded: boolean; location: string } => {
    const normalizedText = (questionText || '').trim().toLowerCase();
    
    // Check in main questions list (by ID and content)
    const inQuestions = questions.some(q => 
      (q.questionBankId === questionId) || (q.id === questionId) || 
      (normalizedText && (q.questionText || '').trim().toLowerCase() === normalizedText)
    );
    
    // Check in question pool (by ID and content)
    const inPool = questionPool.some(q => 
      (q.questionBankId === questionId) || (q.id === questionId) ||
      (normalizedText && (q.questionText || '').trim().toLowerCase() === normalizedText)
    );

    // Check in likert questions (by ID and content)
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (existingExam) {
        ...
        setAttemptWindowDays((existingExam as any).attemptWindowDays || 4);
  ...
  const resetCustomQuestionForm = () => {
    ...
    customQuestionHintRef.current = '';
    customQuestionSolutionRef.current = '';
  ...
  const handleEditQuestion = (index: number) => {
    ...
    customQuestionTextRef.current = convertedQuestionText;
    setCustomEditorKey(k => k + 1);
    ...
    customQuestionChapterRef.current = (question.chapter || '');
    setCustomQuestionSubject(question.subject || '');
    
    // Ensure question's subject is in the dropdown list
    if (question.subject && !subjects.includes(question.subject)) {
    ...
    customQuestionHintRef.current = convertToProperCodeFormat(question.hint);
    customQuestionSolutionRef.current = convertToProperCodeFormat(question.solution);
  ...
  const handleEditPoolQuestion = (index: number) => {
    ...
    customQuestionHintRef.current = convertedHint || '';
    customQuestionSolutionRef.current = convertedSolution || '';
    setCustomQuestionImageUrls(question.imageUrls || []);
  ...
  const handleCreateCustomQuestion = () => {
    ...
    const cleanedQuestionText = cleanEmptyTags(customQuestionTextRef.current);
    ...
    const effectiveChapter = (availableChapters.length === 0 || customQuestionChapter === '__new__') ? customQuestionChapterRef.current : customQuestionChapter;
    if (!effectiveChapter.trim()) {
    ...
    // Create the question object
      console.log('💾 Creating question with customQuestionText:', customQuestionTextRef.current);
      console.log('💾 Has data-latex attribute?', customQuestionTextRef.current.includes('data-latex'));
      console.log('💾 Has data-type="math"?', customQuestionTextRef.current.includes('data-type="math"'));
      
      const cleanedText = cleanEmptyTags(customQuestionTextRef.current);
    ...
    const cleanedHint = cleanEmptyTags(customQuestionHintRef.current.trim());
    const cleanedSolution = cleanEmptyTags(customQuestionSolutionRef.current.trim());
  ...
  const fetchAvailableTags = async () => {
    ...
    const tagSubject = isLikertMode ? 'Personality Assessment' : subject;
    // Use the dedicated tags method — returns ONLY tag names (no question documents loaded
    // into the modal), filtered to the selected class + subject. If class/subject aren't
    // chosen yet, don't load anything; tags populate once a class + subject is selected.
    if (!activeCollegeId || !className || !tagSubject) {
      setAvailableTags([]);
      return;
    }

    try {
      const tags = await firebaseService.getAllTagsForSubject(activeCollegeId, className, tagSubject);
  ...
  const selectAllQuestions = async () => {
    ...
    try {
      ...
      // Filter out questions that are already added to exam or pool (by ID and content)
      const existingQuestionIds = new Set([
        ...questions.map(q => q.questionBankId || q.id),
        ...questionPool.map(q => q.questionBankId || q.id)
      ]);
      const existingTexts = new Set([
        ...questions.map(q => (q.questionText || '').trim().toLowerCase()),
        ...questionPool.map(q => (q.questionText || '').trim().toLowerCase())
      ]);
      
      const selectableQuestions = allQuestions.filter((q: any) => 
        !existingQuestionIds.has(q.id) && !existingTexts.has((q.questionText || '').trim().toLowerCase())
      );
  ...
  const addSelectedQuestions = () => {
    ...
    // Check for duplicates in target list (by ID and by content)
    const existingQuestionIds = new Set(targetList.map(q => q.questionBankId || q.id));
    const existingQuestionTexts = new Set(targetList.map(q => (q.questionText || '').trim().toLowerCase()));
    
    // Check for duplicates in the other list (cross-list checking by ID and content)
    const otherListQuestionIds = new Set(otherList.map(q => q.questionBankId || q.id));
    const otherListQuestionTexts = new Set(otherList.map(q => (q.questionText || '').trim().toLowerCase()));
    ...
    selectedQuestions.forEach(q => {
      ...
      if (existingQuestionIds.has(qId) || existingQuestionTexts.has(qText)) {
        // Already in target list (by ID or same content)
        duplicates.push(q.questionText?.substring(0, 50) + '...' || 'Question');
      } else if (otherListQuestionIds.has(qId) || otherListQuestionTexts.has(qText)) {
        // Already in the other list (by ID or same content)
        crossListDuplicates.push(q.questionText?.substring(0, 50) + '...' || 'Question');
      } else {
        newQuestions.push(q);
        // Track newly added content to catch duplicates within the same batch
        existingQuestionTexts.add(qText);
      }
    });
  ...
  return (
    ...
    <div className={`fixed inset-0 z-[9999] flex items-start justify-end p-2 transition-opacity duration-300 ${
      slideVisible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
    }`}>
      <div 
      ...
      <div 
        className={`relative bg-white shadow-2xl w-[calc(100%-8px)] max-w-[50rem] h-[calc(100%-4px)] flex flex-col overflow-hidden z-10 transform transition-transform duration-500 ease-in-out rounded-2xl ${
          slideVisible ? 'translate-x-0' : 'translate-x-full'
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        <div 
                ...
                <div className="p-4">
                  ...
                  <div className="mt-6 pt-6 border-t-2 border-gray-200">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          ...
                          {/* Flexible Completion — attempt window + per-attempt duration */}
                          <button
                            type="button"
                            onClick={() => setCompletionPolicy('window')}
                            className={`relative overflow-hidden border rounded-xl p-4 transition-all text-left ${
                              completionPolicy === 'window'
                          ...
                          >
                            ...
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-3 ${
                              completionPolicy === 'window' ? 'bg-purple-100' : 'bg-gray-100'
                            }`}>
                              <FontAwesomeIcon icon={faCalendar} className={`text-2xl ${
                                completionPolicy === 'window' ? 'text-purple-600' : 'text-gray-500'
                              }`} />
                            </div>
                            <h4 className={`font-bold text-sm mb-1.5 ${
                              completionPolicy === 'window' ? 'text-purple-900' : 'text-gray-700'
                            }`}>
                              Flexible Completion
                            </h4>
                            <p className={`text-xs leading-relaxed ${
                              completionPolicy === 'window' ? 'text-purple-700' : 'text-gray-600'
                            }`}>
                              Open for a set number of days; once a student starts, they finish within the exam duration
                            </p>
                            <div className="mt-3 space-y-1.5">
                              <div className="flex items-start gap-2">
                                <FontAwesomeIcon icon={faCheckCircle} className={`text-xs mt-0.5 flex-shrink-0 ${
                                  completionPolicy === 'window' ? 'text-purple-600' : 'text-gray-400'
                                }`} />
                                <span className={`text-xs ${completionPolicy === 'window' ? 'text-purple-700' : 'text-gray-600'}`}>
                                  Attempt window in days
                                </span>
                              </div>
                              <div className="flex items-start gap-2">
                                <FontAwesomeIcon icon={faCheckCircle} className={`text-xs mt-0.5 flex-shrink-0 ${
                                ...
                                <span className={`text-xs ${completionPolicy === 'window' ? 'text-purple-700' : 'text-gray-600'}`}>
                                  Timer starts when student starts
                                </span>
                              </div>
                            </div>
                            <div className={`mt-3 inline-block px-2 py-1 rounded text-xs font-semibold ${
                              completionPolicy === 'window' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600'
                            }`}>
                              Open Window
                            </div>
                          </button>
                        ...
                        <div className={`mt-4 p-3 rounded-lg border ${
                          completionPolicy === 'strict' ? 'bg-blue-50 border-blue-200'
                          : completionPolicy === 'flexible' ? 'bg-green-50 border-green-200'
                          ...
                          <div className="flex items-start gap-2">
                            <FontAwesomeIcon 
                              ...
                              className={`text-sm mt-0.5 ${
                                completionPolicy === 'strict' ? 'text-blue-600'
                                : completionPolicy === 'flexible' ? 'text-green-600'
                                : 'text-purple-600'
                              }`}
                            />
                            <div className="flex-1">
                              <p className={`text-xs font-semibold mb-1 ${
                                completionPolicy === 'strict' ? 'text-blue-900'
                                : completionPolicy === 'flexible' ? 'text-green-900'
                                : 'text-purple-900'
                              }`}>
                                {completionPolicy === 'strict' ? 'How Strict Mode Works:'
                                 : completionPolicy === 'flexible' ? 'How Extended Mode Works:'
                                 : 'How Flexible Mode Works:'}
                              </p>
                              <p className={`text-xs leading-relaxed ${
                                completionPolicy === 'strict' ? 'text-blue-700'
                                : completionPolicy === 'flexible' ? 'text-green-700'
                                : 'text-purple-700'
                              }`}>
                                {completionPolicy === 'strict' 
                                  ...
                                  : `The exam stays open for ${attemptWindowDays} day${attemptWindowDays === 1 ? '' : 's'} from the given exam date and time. A student may start any time within that window, but once started, the exam must be completed within the given exam duration.`}
                              </p>
                              {completionPolicy === 'window' && (
                                <div className="mt-2.5 flex items-center gap-2">
                                  <input
                                    type="number"
                                    min={1}
                                    value={attemptWindowDays}
                                    onChange={(e) => setAttemptWindowDays(Math.max(1, parseInt(e.target.value) || 1))}
                                    className="w-24 px-3 py-1.5 border border-purple-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent"
                                  />
                                  <span className="text-sm text-purple-700 font-medium">days open to start</span>
                                </div>
                              )}
                            </div>
                          </div>
              ...
              <div className="space-y-5">
                      ...
                      {availableChapters.length > 0 && customQuestionChapter !== '__new__' ? (
                        ...
                        <div>
                          <input
                            type="text"
                            key={customEditorKey}
                            defaultValue={customQuestionChapter === '__new__' ? '' : customQuestionChapter}
                            onChange={(e) => { customQuestionChapterRef.current = e.target.value; }}
                            className="w-full h-[44px] px-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm text-gray-900 hover:border-gray-300 bg-white transition-all"
                ...
                <div className="border rounded-xl p-4"
                  ...
                  <div className="border border-gray-300 rounded-lg overflow-hidden">
                    <IsolatedCustomEditor
                      key={customEditorKey}
                      initialValue={customQuestionText}
                      valueRef={customQuestionTextRef}
...
{`int solution(int n) {
    ...
    return 0;
                        ...
                        {customStarterCodes.map((sc, scIndex) => {
                          ...
                          return (
                            <div key={scIndex} className="border border-gray-200 rounded-lg p-3 bg-white">
                              ...
                              <IsolatedTextarea key={`sc-${scIndex}-${sc.language}`} initialValue={sc.code} onCommit={(val: string) => { const updated = [...customStarterCodes]; updated[scIndex] = { ...updated[scIndex], code: val }; setCustomStarterCodes(updated); if (scIndex === 0) { setCustomQuestionTestStub(val); } }} placeholder={`// Write starter code for ${sc.language === 'python' ? 'Python' : sc.language === 'cpp' ? 'C++' : sc.language === 'csharp' ? 'C#' : sc.language === 'javascript' ? 'JavaScript' : sc.language === 'typescript' ? 'TypeScript' : sc.language === 'bash' ? 'Bash/Shell' : sc.language.charAt(0).toUpperCase() + sc.language.slice(1)}...`} rows={5} className="w-full px-3 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:border-transparent resize-y text-sm font-mono" style={{ minHeight: '48px', maxHeight: '300px' }} />
                            </div>
                          );
                        })}
                      </div>
                          ...
                          <div key={index} className="flex items-start space-x-2 p-3 bg-gray-50 rounded-lg">
                            <div className="flex-1 space-y-2">
                              <div className="grid grid-cols-2 gap-2">
                                ...
                                />
                                <IsolatedInput
                                  key={`tc-out-${index}-${testCase.expected_output}`}
                                  type="text"
                                  initialValue={testCase.expected_output}
                                  onCommit={(val: string) => {
                                    const newTestCases = [...customQuestionTestCases];
                                    newTestCases[index].expected_output = val;
                                    setCustomQuestionTestCases(newTestCases);
                                  }}
                ...
                <div className="border rounded-xl p-4"
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <textarea
                        key={customEditorKey}
                        defaultValue={customQuestionHint}
                        onChange={(e) => { customQuestionHintRef.current = e.target.value; }}
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-gray-900 mb-1.5">Solution</label>
                        <div className="border border-gray-300 rounded-lg overflow-hidden">
                          <IsolatedCustomEditor
                            key={customEditorKey}
                            initialValue={customQuestionSolution}
                            valueRef={customQuestionSolutionRef}
                ...
                <div className="space-y-4">
                  {questionBankItems.map((question, index) => (
                    <QuestionBankCard
                      ...
                      selectedStarterLang={selectedStarterLang}
                      setSelectedStarterLang={setSelectedStarterLang}
      ...
      {showStudentModal && (
        <div 
          className="fixed inset-0 z-[10002] flex items-start justify-end p-2"
          ...
          >
            ...
            <div className="px-5 py-3 border-b border-gray-200 flex items-center space-x-3 flex-shrink-0 bg-gray-50">
              ...
              <div className="flex-1 relative">
                ...
                <IsolatedDebouncedSearch
                  initialValue={debouncedStudentSearch}
                  onDebouncedChange={setDebouncedStudentSearch}
              ...
              {isLoadingStudents ? (
                ...
                paginatedStudents.map((student) => (
                  <StudentRow
                    key={student.userId}
                    student={student}
                    isSelected={selectedStudentIds.has(student.userId)}
                    onToggle={toggleStudentSelection}
                    avatarGradient={brand.gradients.primary}
                    classColor={brand.colors.primary}
                  />
                ))
              )}
            </div>
