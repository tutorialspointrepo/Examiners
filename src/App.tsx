...
const fetchCounts = async () => {
 ...
 useEffect(() => {
  ...
  // Responsive collapsing based on right panel width
  useEffect(() => {
    const checkCollapse = () => {
      // Skip auto-collapse for Calendar and LeaderBoard (no right panel)
      if (activeItem === ACTIVE_ITEMS.CALENDAR || activeItem === ACTIVE_ITEMS.LEADERBOARD) {
        return;
      }
      if (activeItem === ACTIVE_ITEMS.HALLTICKETS) {
            return currentUser?.userType && 
                   ['teacher', 'admin', 'principal', 'dean', 'system_admin'].includes(currentUser.userType);
      }

      // Calculate actual right panel width considering window size
      const leftWidth = isLeftCollapsed ? 64 : 280;
      const mainWidth = isMainCollapsed ? 64 : 400;
      const calculatedRightWidth = window.innerWidth - leftWidth - mainWidth;
      
      // Update the actual right width state
      setActualRightWidth(calculatedRightWidth);
      
      // If actual right panel width < 700px, collapse left sidebar (unless user manually expanded it)
      if (calculatedRightWidth < 700 && !userInteractedLeft.current) {
        setIsLeftCollapsed(true);
      } else if (calculatedRightWidth >= 700 && !userInteractedLeft.current && !isLeftCollapsed) {
        setIsLeftCollapsed(false);
      }

      // If actual right panel width < 500px, collapse main content (unless user manually expanded it)
      if (calculatedRightWidth < 500 && !userInteractedMain.current) {
        setIsMainCollapsed(true);
      } else if (calculatedRightWidth >= 500 && !userInteractedMain.current && !isMainCollapsed) {
  ...
  }, [isLeftCollapsed, isMainCollapsed, activeItem]);
