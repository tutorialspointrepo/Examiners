/**
 * Modified (c) 2025 The Nuevodevel Team. All rights reserved.
...
var registerSourceHandler = function (videojs) {
    ...
    function Html5Hlsjs(source, tech) {
        ...
        function _isSameTextTrack(track1, track2) {
			...
			//if(track1.label==track2.label && track1.kind==track2.kind) return true;
			//return false;
        ...
        function _filterDisplayableTextTracks(textTracks) {
            ...
            for (var ctx = 0; ctx < textTracks.length; ctx++) {
                if (textTracks[ctx].kind === 'subtitles' || textTracks[ctx].kind === 'captions') {
                    displayableTracks.push(textTracks[ctx]);
                }
            }
		...
		function _updateRemoteTrackList(dt) {
	
		    var isAdded=false;
		    var remoteTracks = _player.remoteTextTracks();
			...
			if(remoteTracks.length>0) {
				...
				for (let rt=0; rt<remoteTracks.length;rt++) {
					let rm=remoteTracks[rt];
					for(let td=0;td<dt.length;td++) {
		
					   
					  if(dt[td].label===rm.label && dt[td].kind===rm.kind && dt[td].language===rm.language) {
						isAdded=true;
						break;
					  }
				...
				let tracks = _player.remoteTextTracks();
				    var i = tracks.length || 0;
				while (i--) {
				  _player.removeRemoteTextTrack(tracks[i]);
				}
        ...
        function _updateTextTrackList() {
			...
			if(_player.remoteTextTracks().length>0) return;
			...
			if(Array.isArray(playerTextTracks)) {
				if(playerTextTracks.length>0) {
					for (var ctx = 0; ctx < playerTextTracks.length; ctx++) {
						var isAdded = false;
						
						if(displayableTracks.length>0) {
						   for (var cs=0;cs<displayableTracks.length;cs++) {
								let dt = displayableTracks[cs];
								let pt = playerTextTracks[ctx];
				
								if(dt.label===pt.label && dt.kind===pt.kind && dt.language==pt.language) {
									isAdded=true;
									break;
								}
						   }						   
						...
						if (!isAdded) {
							var hlsjsTextTrack = displayableTracks[ctx];
			...
			if(!hlstracks && displayableTracks.length>0) {

				_updateRemoteTrackList(displayableTracks);
			    hlstracks=true;
			}
		...
		_player.on("timeupdate", function() {
				...
				document.getElementById("output").value="IPHONE:"+videojs.browser.IS_IOS;
