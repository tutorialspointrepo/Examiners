// restore_default_user.js
// Restore the default system super admin user
const admin = require('firebase-admin');

const serviceAccount = {
  "type": "service_account",
  "project_id": "examiners-app",
  "private_key_id": "26f015b6e61cd7277a758c12582e9a9d8af9bbc5",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@examiners-app.iam.gserviceaccount.com",
  "client_id": "111371224895314632304",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40examiners-app.iam.gserviceaccount.com"
};

admin.initializeApp({
  credential: admin.credential.cert(serviceAccount)
});

const db = admin.firestore();

async function restoreDefaultUser() {
  try {
    const uid = 'RyIDJMC8PVcju4ADd7i5lB3ROGq1';

    // Step 1: Check if Auth user still exists
    console.log('🔍 Checking if Auth user still exists...');
    let authUser = null;
    try {
      authUser = await admin.auth().getUser(uid);
      console.log('✅ Auth user exists:');
      console.log(`   UID: ${authUser.uid}`);
      console.log(`   Phone: ${authUser.phoneNumber || 'N/A'}`);
      console.log(`   Email: ${authUser.email || 'N/A'}`);
    } catch (err) {
      if (err.code === 'auth/user-not-found') {
        console.log('⚠️  Auth user was also deleted. Will recreate.');
        console.log('⚠️  UPDATE the phone/email/password below before running!');
      } else {
        throw err;
      }
    }

    // Step 2: If Auth user was deleted, recreate it
    if (!authUser) {
      console.log('\n🔧 Recreating Auth user...');
      // ⚠️ UPDATE these values if needed
      authUser = await admin.auth().createUser({
        uid: uid,
        email: 'mohtashim@tutorialspoint.com',
        emailVerified: true,
        password: 'Super@123456',
        displayName: 'mohtashim'
      });
      console.log('✅ Auth user recreated with UID:', authUser.uid);
    }

    // Step 3: Restore Firestore document
    console.log('\n🔧 Restoring Firestore user document...');

    const userData = {
      userId: uid,
      fullName: 'mohtashim',
      email: authUser.email || 'mohtashim@tutorialspoint.com',
      phone: authUser.phoneNumber || '',
      userType: 'system_admin',
      collegeId: 'TPX',
      board: 'TPX',
      status: 'active',
      permissions: {
        canEvaluate: true,
        canViewReports: true,
        canManageUsers: true,
        canManageColleges: true,
        canManageRooms: true,
        canManageClasses: true,
        canManageSubjects: true,
        canManageBoards: true,
        canManageExamTypes: true,
        canManageSystem: true,
        canAccessAllColleges: true,
        level: 'system'
      },
      restoredAt: admin.firestore.FieldValue.serverTimestamp(),
      restoredReason: 'Default system super admin — accidentally deleted during orphan cleanup (was collegeId: default)'
    };

    await db.collection('users').doc(uid).set(userData, { merge: true });

    console.log('✅ Firestore document restored!\n');
    console.log('═══════════════════════════════════════');
    console.log('  RESTORED DEFAULT SYSTEM USER');
    console.log('═══════════════════════════════════════');
    console.log(`  UID:        ${uid}`);
    console.log(`  Name:       ${userData.fullName}`);
    console.log(`  Email:      ${userData.email}`);
    console.log(`  UserType:   ${userData.userType}`);
    console.log(`  CollegeId:  ${userData.collegeId} (was: default)`);
    console.log('═══════════════════════════════════════');

    process.exit(0);
  } catch (error) {
    console.error('❌ Error:', error.message);
    process.exit(1);
  }
}

restoreDefaultUser();
